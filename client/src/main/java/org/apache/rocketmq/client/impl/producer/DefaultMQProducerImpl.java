...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                /* 如果生产组不为CLIENT_INNER_PRODUCER_GROUP，则将实例名更新为PID*/
    ...
    private SendResult sendDefaultImpl(
        ...
        // 获取topic路由信息[先本地topicPublishInfoTable，后去nameServer获取]
        TopicPublishInfo topicPublishInfo = this.tryToFindTopicPublishInfo(msg.getTopic());
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            MessageQueue mq = null;
            Exception exception = null;
            SendResult sendResult = null;
            // 设置重试次数： 同步为1+配置的重试次数，异步为1
    ...
    public TransactionSendResult sendMessageInTransaction(final Message msg,
        ...
        try {
            // 发送半消息--暂不能投递的消息(发送方已经将消息成功发送到了 MQ 服务端，但是服务端未收到生产者对该消息的二次确认
            // 此时该消息被标记成“暂不能投递”状态)
        ...
        switch (sendResult.getSendStatus()) {
            case SEND_OK: {
                try {
                    ...
                    // 调用本地事务--实现LocalTransactionExecuter接口的类
